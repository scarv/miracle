...
/*!
@ingroup experiments-memory-bus-width-ld-halfwords
@{
*/
...
uint16_t zeros     [DLEN]; //!< Array of zero'd halfwords
uint16_t din_fixed       ; //!< TTest fixed input value.
uint16_t din_rand        ; //!< TTest random input value.
uint16_t din       [DLEN]; //!< Array of zeros except for TTest variable.
...
{"din" , 2, &din_rand, &din_fixed, SCASS_FLAGS_TTEST_IN},
{"idx" , 1, &dindex  , &dindex   , SCASS_FLAG_INPUT    },
{"off" , 1, &doffset , &doffset  , SCASS_FLAG_INPUT    }
};

//! Declaration for the experiment payload function in load-half.S
...
void experiment_setup_scass(
    ...
    cfg -> experiment_name       = "memory-bus/bus-width-ld-halfword";
