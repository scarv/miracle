...
# Usage:
#
#   $> ./experiments/memory-bus/per-byte/scale-ttest-all.sh <target> <serial port>
#

TT_NAME=per-byte-10k
...
#
# Build
make -B build_$1_memory-bus-per-byte

#
# Program
make -B program_$1_memory-bus-per-byte USB_PORT=$2
...
${UAS_ROOT}/experiments/memory-bus/per-byte/ttest_graph.sh $1 ${TT_NAME}
