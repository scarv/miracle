...
```sh
$> ./target/sakurax_mb8/flow/recreate-vivado-project.sh
```
...
bitfile over to `$UAS_ROOT/target/sakurax_mb8/program/bitstream.bit` to use
the auto-program make targets described above.

- You may also need to re-generate the memory map information file
  `target/sakurax_mb8/program/sakurax_mb8.mmi` to make sure that the ELF
